...
echo "<h2>Setting up Rentify Database...</h2>";
...
$tables = [
    ...
    "rental_orders" => $create_rental_orders_table
];
...
if ($row['count'] == 0) {
    ...
    foreach ($customers as $customer) {
        ...
        if (mysqli_stmt_execute($stmt)) {
            echo "<p style='color: green;'>✓ Customer '{$customer[0]} {$customer[1]}' added</p>";
        }
    }
    ...
    foreach ($orders as $order) {
        ...
        if (mysqli_stmt_execute($stmt)) {
            echo "<p style='color: green;'>✓ Rental order added</p>";
        }
    }
...
echo "<a href='login.php' style='color: #0066cc; text-decoration: none;'>👉 Go to Login Page</a>";
echo "</div>";
?>
